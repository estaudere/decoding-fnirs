...
from non_rest_classifier.dataset import load_datasets, FNIRSDataset
from non_rest_classifier.fnirsnet import fNIRSNet, LabelSmoothing
...
train_dataset, test_dataset = load_datasets(SUBJECT_NAME, split=SPLIT, sliding_windows=True, window_size=30, stride=5) 
train_loader = DataLoader(train_dataset, batch_size=BATCH_SIZE, shuffle=True, drop_last=True)
test_loader = DataLoader(test_dataset, batch_size=BATCH_SIZE, shuffle=False, drop_last=True)
logger.info(f"train dataset size: {len(train_dataset)}, test dataset size: {len(test_dataset)}")

# Initialize model
model = fNIRSNet(train_dataset.num_classes, train_dataset.num_timesteps, train_dataset.num_channels, num_DHRConv=4, num_DWConv=16)
model.to(DEVICE)
criterion = LabelSmoothing(0.5)
